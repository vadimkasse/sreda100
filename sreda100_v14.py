...
"""
SREDA100 v14
- Bold/Heavy/Black fonts only, correct ttc indices
- Font size max 200px
- earthquake + gravity: primary pass only
- Pass 2 t: 0.10–0.25 (subtle)
- Filename: effect1_t1_effect2_t2_font_fs_ls_day_date_s.png
"""
...
BOLD_FONTS = [
    ...
    ("/System/Library/Fonts/Supplemental/Baskerville.ttc", 1, "baskerville_bold"),
    ...
    ("/System/Library/Fonts/Supplemental/AmericanTypewriter.ttc", 2, "typewriter_bold"),
    ("/System/Library/Fonts/Supplemental/AmericanTypewriter.ttc", 5, "typewriter_cond_bold"),
...
# Pass 1: all effects including earthquake + gravity
PRIMARY_EFFECTS = ["chaos", "earthquake", "twist", "shatter", "noise_flow", "gravity"]
PRIMARY_WEIGHTS = [5, 4, 3, 4, 2, 3]

# Pass 2: no earthquake, no gravity
...
def generate(day, seed=None, glitch=False):
    ...
    # Var 2: size (max 200) + spacing
    target_fraction = rng.uniform(0.20, 0.95)
    font_size = fit_font_to_width(day, font_path, font_index, target_fraction, WIDTH)
    ...
    t1 = rng.uniform(*T_RANGES[e1])

    # Pass 2 — different from pass 1, always subtle
    ...
    print(f"✅ {e1:12s}{int(t1*100):2d} + {e2:10s}{int(t2*100):2d} | {font_label:28s} fs={font_size:3d} ls={letter_spacing:3d}")
    return out_path
...
if __name__ == "__main__":
    ...
    print(f"Generating {args.batch} × {args.day} (v14, bold fonts, 2-pass)...")
